...
function scheduleCheckThread(player){
	if(!player.playing)return;
	if(player.checkThread){
		clearInterval(player.checkThread);
	}
	player.checkThread=setTimeout(checkThread,1000,player);
};
function checkThread(player){
...
WebRtcStreamer.prototype.disconnect = function() {
	...
	if(this.checkThread){
		clearInterval(this.checkThread);
	}
}
